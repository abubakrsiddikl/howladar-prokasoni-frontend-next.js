...
export default function CampaignCheckoutForm({
  ...
  // 💡 isDeliveryFree true হলে চার্জ 0, না হলে ঢাকার ভেতরে 60 এবং বাইরে 120
  const deliveryCharge = campaign.isDeliveryFree
    ? 0
  ...
  useEffect(() => {
    if (state?.success) {
      ...
      toast.error(state.message || "অর্ডার সম্পন্ন করতে সমস্যা হয়েছে।");
    }
  }, [state, router, campaign, totalAmount]);
  ...
  return (
    ...
    >
          ...
          <span className="font-semibold text-gray-900">
            {campaign.isDeliveryFree ? (
              <span className=" font-bold">
                সারা বাংলাদেশে ডেলিভারি চার্জ একদম ফ্রি!
              </span>
            ) : (
              `৳ ${deliveryCharge}`
            )}
      ...
      <Button
        ...
        className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-5 text-base sm:text-lg rounded-lg shadow-md transition-all cursor-pointer mt-4"
      >
        {isPending ? "অর্ডার নেওয়া হচ্ছে..." : `Order Now (৳ ${totalAmount})`}
      </Button>
    </form>
  );
}
