...
export default function OrderDetailsCard({
  ...
  // totals — only meaningful for regular (multi-item book) orders
  const subTotal = order.totalAmount - 120;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Order Summary — branches on order type */}
        <div className="p-4 rounded-lg border bg-white">
          <h2 className="text-lg font-bold mb-4">Order Summary</h2>
          <div className="border-t border-dashed border-[#708dbf] my-4" />
          ...
          {isGuestCampaignOrder ? (
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div className="grow">
                  ...
                  <p className="text-[#4d3633] font-semibold text-sm sm:text-base md:text-lg">
                    Tk. {campaign?.campaignPrice}
                  </p>
                  {/* {order.quantity && (
                    <p className="text-sm text-gray-600">
                      Qty: {order.quantity}
              ...
              {/* Admin/manager-only: trace this order back to its campaign */}
              {isAdminOrManager && (
                <Link
                  href={`/campaign/${campaign?.slug}`}
                  className="inline-block text-sm text-blue-600 hover:underline"
                >
                  এই অর্ডারটি যে ক্যাম্পেইন থেকে এসেছে তা দেখুন →
                </Link>
              )}
            ...
            // Regular order — unchanged, original book cart rendering.
            <div className="h-[200px] space-y-4 overflow-y-scroll">
              {order.items?.map((item, idx: number) => (
                <div key={idx}>
                  <div className="flex items-center gap-4">
